...
export async function updateBooking(selfid: string, date: Date, starttime: string, endtime: string, path: string) {
    ...
    try {
        ...
        const parkingLocationData = await ParkingLocationModel.findById(originalBooking.locationid).lean();
        if (!parkingLocationData) {
            throw new Error('Parking location not found');
        }
        const parkingLocation = parkingLocationData as unknown as ParkingLocation;
